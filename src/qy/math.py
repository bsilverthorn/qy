...
def ln_gamma(x):
    ...
    This implementation is adapted from the same Cody & Stoltz netlib code that
    everyone builds their implementation from.
    """
    ...
    if (x <= 0.5) | ((x > a) & (x <= 1.5)):
        if(x <= 0.5):
            ...
            if x + 1.0 == 1.0:
                # x is less than machine epsilon
                return result
        else:
            result = 0.0
            x      = (x - 0.5) - 0.5
        ...
        return result + x * (d1 + x * (xnum / xden))
    elif (x <= a) | ((x > 1.5) & (x <= 4)):
        if x <= a:
            result = -log(x)
            x      = (x - 0.5) - 0.5
        ...
        return result + x * (d2 + x * (xnum / xden))
    elif x <= 12:
        x    -= 4
        xnum  = 0
        xden  = -1
